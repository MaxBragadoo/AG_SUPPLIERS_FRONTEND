<template>
      ...
      <v-flex>
        <v-toolbar flat color="white">
         ...
         <!-- <v-btn v-if="verNuevo == 0" @click="listar()" color="primary" dark class="mb-2">Buscar</v-btn>-->
          <v-spacer></v-spacer>
          <v-btn v-if="verNuevo == 0" @click="mostrarNuevo" color="primary" dark class="mb-2">Nueva Compra</v-btn>
          <v-spacer></v-spacer>
            ...
            <v-col cols="12">
              <v-data-table :headers="cabeceraDetalles" :items="detalles" :hide-default-footer="true" class="elevation-1">
                <template v-slot:item="{ item }">
                  <tr>
                    <td class="text-center">
                      <v-icon small @click="eliminarDetalle(detalles, item)">mdi-delete</v-icon>
                    </td>
  ...
  <script>  
    ...
    export default {
      ...
      //SLP  const idusuario = ref('');
        const comprobantes = ref(['PEDIMIENTO', 'FACTURA', 'BOLETA', 'TICKET', 'GUIA']);
        const tipo_movimientos = ref(['E01', 'S01', 'E10']);
        ...
        const articulos = ref([]);
        const texto = ref('');
        const verArticulos = ref(0);
        const verDet = ref(0);
        const valida = ref(0);
        const validaMensaje = ref([]);
        const adModal = ref(0);
        const adAccion = ref(0);
        const adNombre = ref('');
        const adId = ref('');
        const detalles = ref([]);
    ...
    const activarDesactivarMostrar = (accion, item) => {
      adModal.value = 1;
      adNombre.value = item.num_comprobante;
